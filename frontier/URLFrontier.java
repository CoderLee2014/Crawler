...
public class URLFrontier {
	//URL initials
		...
		public  void createPriorityQueue() throws MalformedURLException{
		...
		public URL pop(){
			System.out.println("crawler "+ Thread.currentThread().getId()%100+" is poping");
			if(heap_time.size()==0){
				router();
			}
			if(heap_time.size()==0){
			...
			if(backqueue.get(backpos).isEmpty()){
				System.out.println("enqueue a url in backqueue...");
				router();
			...
			System.out.println("crawler "+ Thread.currentThread().getId()%100+" finished poping");
			return url;
		}
		...
		private void router() {
			...
			while(priority>0&&limit>0){
				...
				if(size == 0){ 
					System.out.println("t1 queue is null");
					continue;
				}
				...
				while(!list_queues_t1.get(priority).isEmpty()&&size>0){
					...
					if(hote_deque.containsKey(url.getHost())){
						...
						if(posB==maxBack){
							list_queues_t1.get(priority).pop();
							list_queues_t1.get(priority).push(url);
							continue;
						}
			...
			System.out.println("crawler "+ Thread.currentThread().getId()%100+" finished routing");
		}
